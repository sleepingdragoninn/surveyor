...
public class RegionSummary {
	...
	public static RegionSummary fromEmpty(File folder, WorldSummary summary, RegionPos regionPos) {
		return new RegionSummary(summary, new File(folder, "c.%d.%d.dat".formatted(regionPos.x(), regionPos.z())), regionPos, new ChunkSummary[RegionPos.CHUNK_SIZE][RegionPos.CHUNK_SIZE], new BitSet(RegionPos.CHUNK_AREA), new RegistryPalette<>(summary.manager().getOrThrow(RegistryKeys.BIOME)), new RegistryPalette<>(summary.manager().getOrThrow(RegistryKeys.BLOCK)));
	}
	...
	protected void readNbt(RegionPos pos, boolean bitsOnly) {
		Registry<Biome> biomeRegistry = summary.manager().getOrThrow(RegistryKeys.BIOME);
		Registry<Block> blockRegistry = summary.manager().getOrThrow(RegistryKeys.BLOCK);
		...
		this.biomePalette = new RegistryPalette<>(summary.manager().getOrThrow(RegistryKeys.BIOME));
		this.blockPalette = new RegistryPalette<>(summary.manager().getOrThrow(RegistryKeys.BLOCK));
