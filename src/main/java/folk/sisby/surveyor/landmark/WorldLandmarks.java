		...
		if (landmarks.getKeys().stream().anyMatch(k -> k.contains(":"))) { // 0.X
			...
			try {
				for (String key : landmarks.getKeys()) {
					NbtCompound type = landmarks.getCompound(key).orElse(new NbtCompound());
					Identifier typeId = Identifier.tryParse(key);
					for (String coords : type.getKeys()) {
						NbtCompound landmark = type.getCompound(coords).orElse(new NbtCompound());
						...
						Identifier id = landmark.getString("texture").map(Identifier::tryParse).orElse(typeId).withSuffixedPath((dye == null ? "" : "/" + dye.getId()) + "/" + pos.getX() + (pos.getY() == 0 ? "" : "/" + pos.getY()) + "/" + pos.getZ());
