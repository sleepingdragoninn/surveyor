...
public class LandmarkComponentTypes {
	...
	public static final LandmarkComponentType<Text> NAME = register("name", TextCodecs.CODEC, t -> Text.literal("\"").append(t.copy().formatted(Formatting.GREEN)).append(Text.literal("\"")));
	public static final LandmarkComponentType<List<Text>> LORE = register("lore", Codec.list(TextCodecs.CODEC), l -> Texts.join(l, Text.literal("\" | \""), t -> t.copy().formatted(Formatting.GREEN)));
	public static final LandmarkComponentType<Integer> COLOR = register("color", Codec.INT, i -> Text.literal("#").setStyle(Style.EMPTY.withColor(0xFFFFFF & i)).append(Text.literal(Integer.toHexString(0xFFFFFF & i).toUpperCase()).formatted(Formatting.GOLD)));
